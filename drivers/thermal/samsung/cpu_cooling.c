...
exynos_cpufreq_cdev_register(struct exynos_cpufreq_cooling_device *cpufreq_cdev)
{
	...
	size = of_thermal_get_ntrips(cpufreq_cdev->tz);
	for (i = 0; i < size; i++) {
				 ...
				 unsigned long state)
{
	...
	//s32 pprev_temp, prev_temp, cur_temp;
	s64 pprev_data, prev_data, cur_data, coef;
#endif
...
#if IS_ENABLED(CONFIG_EXYNOS_EA_DTM)
	exynos_acpm_tmu_get_pi_params(cpufreq_cdev->tz->id, TMU_DATA_PPREV_DATA, &pprev_data);
	exynos_acpm_tmu_get_pi_params(cpufreq_cdev->tz->id, TMU_DATA_PREV_DATA, &prev_data);
	exynos_acpm_tmu_get_pi_params(cpufreq_cdev->tz->id, TMU_DATA_CUR_DATA, &cur_data);
	exynos_acpm_tmu_get_pi_params(cpufreq_cdev->tz->id, TMU_DATA_CUR_COEF, &coef);

	dbg_snapshot_thermal(cdev->type, state, cpufreq_cdev->freq_table[state].frequency / 1000, 0);
	ret = freq_qos_update_request(&cpufreq_cdev->qos_req,
			cpufreq_cdev->freq_table[state].frequency);

	if (ret == 1)
		ret = 0;
	dbg_snapshot_thermal(cpufreq_cdev->cdev->type, pprev_data & 0xFFFFFFFF,
			(u64)pprev_data >> 32, 1);
	dbg_snapshot_thermal(cpufreq_cdev->cdev->type, prev_data & 0xFFFFFFFF,
			(u64)prev_data >> 32, 2);
	dbg_snapshot_thermal(cpufreq_cdev->cdev->type, cur_data & 0xFFFFFFFF,
			(u64)cur_data >> 32, coef);
#else
				   ...
				   char *tz_name)
{
	...
	for (i = 0; i < function->num_of_range; ++i) {
		...
		if (function->range_list[i].max_frequency == 2600000 ||
			...
			function->range_list[i].max_frequency == 2112000 ||
			function->range_list[i].max_frequency == 2016000 ||
			function->range_list[i].max_frequency == 1920000) {
		
			function->range_list[i].max_frequency = 3000000; // BIG core max OC
		} else if (function->range_list[i].max_frequency == 2002000 ||
				   function->range_list[i].max_frequency == 1536000) {
		
			function->range_list[i].max_frequency = 2200000; // LITTLE core max OC
		} else if (function->range_list[i].max_frequency == 960000) {
		
			function->range_list[i].max_frequency = 2002000; // Bump low-end LITTLE up
		}
